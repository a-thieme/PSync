...
void
FullProducer::scheduleSyncInterest(bool useJitter)
{
	// If there's an existing scheduled event, cancel it
	if (m_scheduledSyncInterestEvent) {
		m_scheduledSyncInterestEvent.cancel();
	}

	// Determine the delay for the next sync interest

	ndn::time::milliseconds delay = useJitter ? ndn::time::milliseconds(m_jitter(m_rng)) :
		m_syncInterestLifetime / 2 + ndn::time::milliseconds(m_jitter(m_rng));
	

	NDN_LOG_TRACE("Scheduling sync interest after " << delay);
 	NDN_LOG_TRACE("send sync interest called with schedule");
	// Schedule the next sync interest
